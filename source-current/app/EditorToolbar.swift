...
struct EditorToolbar: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            tool("bold", title: L10n.tr("Fett"), action: controller.toggleBold)
            tool("italic", title: L10n.tr("Kursiv"), action: controller.toggleItalic)
            separator
            tool("list.bullet", title: L10n.tr("Aufzählung"), action: controller.toggleBullets)
            tool("list.number", title: L10n.tr("Nummerierte Liste"), action: controller.toggleNumbering)
            separator
            tool("textformat", title: L10n.tr("Formatierung entfernen"), action: controller.clearFormatting)
            Spacer(minLength: 8)
            Menu {
            ...
            .accessibilityValue(L10n.tr("{0} Prozent", controller.zoomPercent))
        }
