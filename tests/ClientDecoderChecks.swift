...
extension OpenAIClient {
    static func runDecoderChecks() throws {
        ...
        let payment = try decodePaymentSuggestion("```json\n{\"recipient\":null,\"iban\":null,\"bic\":null,\"amount\":\"123.45\",\"currency\":\"EUR\",\"purpose\":null,\"confidence\":\"low\"}\n```")
        precondition(payment.iban == nil && payment.amount == "123.45" && payment.currency == "EUR")
        let invalid: [() throws -> Void] = [
            ...
            { _ = try decodePaymentSuggestion("not JSON") }
        ]
        for operation in invalid {
            do {
                try operation()
                fatalError("Expected the existing API error")
            } catch {
                precondition(error is APIError)
            }
        }
        ...
        print("PASS: 14 client decoder and error cases")
    }

    static func runFileIOCheck() {
        let client = OpenAIClient()
        let done = DispatchSemaphore(value: 0)
        let absentFile = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString).appendingPathExtension("PDF")
        client.uploadFile(apiKey: "unused-test-key", url: absentFile) { result in
            precondition(!Thread.isMainThread, "PDF preparation blocked the UI thread")
            guard case .failure(let error) = result, error is APIError else {
                fatalError("Expected an unreadable-file error before any network call")
            }
            done.signal()
        }
        precondition(done.wait(timeout: .now() + 5) == .success)
        print("PASS: PDF preparation reports failure off the UI thread, without API access")
    }
}
...
struct ClientCheckRunner {
    static func main() throws {
        try OpenAIClient.runDecoderChecks()
        OpenAIClient.runFileIOCheck()
    }
}
